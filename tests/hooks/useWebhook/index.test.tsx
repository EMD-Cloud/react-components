...
describe('useWebhook Hook Tests', () => {
  ...
  it('should return null webhook instance when SDK is not available', async () => {
    ...
    const { result } = renderHook(() => useWebhook(), { wrapper: failingWrapper })
    
    expect(result.current.webhook).toBeNull()
  })

  it('should throw error when SDK is not initialized for call', async () => {
    // Mock the EmdCloud constructor to fail
    const { EmdCloud } = await import('@emd-cloud/sdk')
    vi.mocked(EmdCloud).mockImplementationOnce(() => {
      throw new Error('SDK initialization failed')
    ...
    await act(async () => {
      await expect(
        result.current.callWebhook('test', { method: 'POST' })
      ).rejects.toThrow('SDK not initialized. Make sure @emd-cloud/sdk is installed as a peer dependency.')
    })
  })

  it('should throw error when SDK is not initialized for POST call', async () => {
    // Mock the EmdCloud constructor to fail
    const { EmdCloud } = await import('@emd-cloud/sdk')
    vi.mocked(EmdCloud).mockImplementationOnce(() => {
      throw new Error('SDK initialization failed')
    ...
    await act(async () => {
      await expect(
        result.current.callWebhook('test', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  it('should throw error when SDK is not initialized for GET call', async () => {
    // Mock the EmdCloud constructor to fail
    const { EmdCloud } = await import('@emd-cloud/sdk')
    vi.mocked(EmdCloud).mockImplementationOnce(() => {
      throw new Error('SDK initialization failed')
    ...
    await act(async () => {
      await expect(
        result.current.callWebhook('test', { method: 'GET' })
      ).rejects.toThrow('SDK not initialized. Make sure @emd-cloud/sdk is installed as a peer dependency.')
    })
  })
